...
func printUsage() {
	fmt.Fprintf(os.Stderr, `rpt - ROKFOSS 패키지 관리자 (%s)

apt 가 없는 Linux 환경에서 ROKFOSS 저장소의 패키지를 관리합니다.
...
환경 변수:
	RPT_ROOT            설치 루트
	RPT_STATEDIR        상태 파일과 목록 캐시 위치 (기본: %s)
	RPT_CACHEDIR        deb 다운로드 캐시 위치 (기본: %s)
